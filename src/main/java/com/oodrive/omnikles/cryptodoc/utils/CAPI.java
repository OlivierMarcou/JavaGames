...
public class CAPI {
	...
	public byte[] decryptMessage(byte[] binaryCertificate, byte[] crypted, char[] password) throws Exception{
        ...
        byte[] data = null;
		try {
			data = initDLL.mscapiDLL.decryptMessage(crypted, binaryCertificate);
		} catch (Exception e) {
        ...
        System.out.println("Got error code from microsoft capi wrapper: " + initDLL.mscapiDLL.getLastErrorCode());
        return data;
	}
